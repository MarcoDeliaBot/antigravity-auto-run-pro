                ...
                ws.on('open', () => {
                    ws.send(JSON.stringify({
                        id: 1, method: 'Runtime.evaluate', params: {
                            expression: 'Array.from(document.querySelectorAll("*")).filter(e => e.textContent && e.textContent.trim() === "Accept all" && e.children.length === 0).map(e => ({tag: e.tagName, html: e.outerHTML, parent: e.parentElement ? e.parentElement.outerHTML : null}))',
                            returnByValue: true
                        }
                    }));
                });
